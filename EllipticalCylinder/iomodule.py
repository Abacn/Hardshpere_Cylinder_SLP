...
def readParameter(filename='config.json'):
    ...
    with open(filename) as data_file:
        ...
        data["Ra"] = data["diameter"]*0.5
        data["Rb"] = data["Ra"]/data["aspectRatio"]
    return data


def writeCoords(coords, boxNow, diameter=1.0, filename=None):
    ...
    results.append('Sphere diameter: %g' % (diameter))
...
def readCoords(filename):
    ...
    with open(filename, 'r') as fin:
        ...
        diameter = float(fin.readline().split(':')[1])  # particle diameter
        _ = fin.readline()
        corrdstrlst = [np.fromstring(s.strip(), sep=' ') for s in fin.readlines()]
        coords = np.array(corrdstrlst)

    return coords, boxNow, diameter

def readBoxSize(filename):
    """Get box size parameters but do not read the whole coordinates"""
    with open(filename, 'r') as fin:
        line = fin.readline()  # Cylinder size
        boxNow = [float(i) for i in re.search(r'\((.+)\)', line).group(1).split(',')]
        N = int(fin.readline().split(':')[1])  # Number of particle
        diameter = float(fin.readline().split(':')[1])  # particle diameter
    return N, boxNow, diameter
